...
# Path to LAMOST medium resolution catalogue
file_path = r"C:\Users\21rgo\Downloads\dr9_v2.0_MRS_catalogue.csv.gz"
...
# and it has columns named 'alpha_m_lasp' and 'feh_cnn'
...
# Extracting the data
feh_cnn = df['feh_cnn']
alpha_m_lasp = df['alpha_m_lasp']

# Creating the scatter plot
plt.figure(figsize=(10, 6))
plt.scatter(feh_cnn, alpha_m_lasp, s=1, color='blue', alpha=0.5)

# Adding labels and title
plt.xlabel('FeH CNN')
plt.ylabel('Alpha_m_lasp')
plt.title('Scatter Plot of Alpha_m_lasp vs FeH CNN')
...
X = np.column_stack((feh_cnn, alpha_m_lasp))
#X = X[:len(df)]
df = df[:len(X)]
...
alpha_m = df.dropna(subset=['alpha_m_lasp'])   #metallicity
...
#not numeric inputs are set aside
...
#we find one value for expected amunt of n in c and one for c in n
...
import pandas as pd     #I think not what I want??
from sklearn.impute import KNNImputer
from sklearn.cluster import KMeans
import matplotlib.pyplot as plt
import numpy as np
...
# Select the columns for clustering
X = np.column_stack((dffeh, df_alpha_m_lasp))
#X = X[:len(df)]
df = df[:len(X)]
# Choose the number of clusters (you can adjust this)
num_clusters = 6
...
# Visualize the clusters
plt.scatter(dffeh,df_alpha_m_lasp, c= df['cluster'], cmap='viridis',s=1)
plt.title('K-means Clustering')
plt.xlabel('dffeh')
plt.ylabel('df_alpha_m_lasp')
plt.show()
